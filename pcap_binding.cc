...
public:
  ...
  static void Init(Handle<Object> target)
  {
    ...
    s_ct->SetClassName(String::NewSymbol("PcapSession"));

    NODE_SET_PROTOTYPE_METHOD(s_ct, "OpenLive", OpenLive);
    NODE_SET_PROTOTYPE_METHOD(s_ct, "OpenOffline", OpenOffline);
    NODE_SET_PROTOTYPE_METHOD(s_ct, "Fileno", Fileno);
    NODE_SET_PROTOTYPE_METHOD(s_ct, "Dispatch", Dispatch);
    NODE_SET_PROTOTYPE_METHOD(s_ct, "Close", Close);
    NODE_SET_PROTOTYPE_METHOD(s_ct, "Stats", Stats);

    target->Set(String::NewSymbol("session"), s_ct->GetFunction());
  }
  ...
  {
    HandleScope scope;
    PcapSession* hw = new PcapSession();
    hw->Wrap(args.This());
    return args.This();
  }  
  
  static Handle<Value> Open(bool live, const Arguments& args)
  {
      HandleScope scope;
      char errbuf[PCAP_ERRBUF_SIZE];
      PcapSession* sess = ObjectWrap::Unwrap<PcapSession>(args.This());
      ...
      if (live) {
          ...
          if (pcap_set_buffer_size(sess->pcap_handle, buffer_size) != 0) {
          ...
          if (pcap_set_timeout(sess->pcap_handle, 1000) != 0) {
          ...
          // if (pcap_set_rfmon(sess->pcap_handle, 1) != 0) {
          //     return ThrowException(Exception::Error(String::New(pcap_geterr(sess->pcap_handle))));
          // }
  
          if (pcap_activate(sess->pcap_handle) != 0) {
              return ThrowException(Exception::Error(String::New(pcap_geterr(sess->pcap_handle))));
          }
      } else {
          // Device is the path to the savefile
          sess->pcap_handle = pcap_open_offline((char *) *device, errbuf);
          if (sess->pcap_handle == NULL) {
              return ThrowException(Exception::Error(String::New(errbuf)));
          }
      }
  
      if (pcap_setnonblock(sess->pcap_handle, 1, errbuf) == -1) {
          return ThrowException(Exception::Error(String::New(errbuf)));
      }
  
      // TODO - if filter is empty, don't bother with compile or set
      if (pcap_compile(sess->pcap_handle, &sess->fp, (char *) *filter, 1, sess->net) == -1) {
          return ThrowException(Exception::Error(String::New(pcap_geterr(sess->pcap_handle))));
      }
  
      if (pcap_setfilter(sess->pcap_handle, &sess->fp) == -1) {
          return ThrowException(Exception::Error(String::New(pcap_geterr(sess->pcap_handle))));
      }
      ...
      #include <net/bpf.h>
      int fd = pcap_get_selectable_fd(sess->pcap_handle);
      int v = 1;
      ioctl(fd, BIOCIMMEDIATE, &v);
      // TODO - check return value
  #endif
  
      int link_type = pcap_datalink(sess->pcap_handle);
  ...
  static Handle<Value> OpenLive(const Arguments& args)
  {
      return Open(true, args);
  }
  
  static Handle<Value> OpenOffline(const Arguments& args)
  {
      return Open(false, args);
  }
  
  
  static Handle<Value> Fileno(const Arguments& args)
  { 
      HandleScope scope;     
      PcapSession* sess = ObjectWrap::Unwrap<PcapSession>(args.This());
  ...
  static void PacketReady(u_char *callback_p, const struct pcap_pkthdr* pkthdr, const u_char* packet) {
      ...
      PcapSession* sess = (PcapSession*)callback_p;
      Local<Function> *callback = (Local<Function>*)sess->callback;
  ...
  static Handle<Value> Dispatch(const Arguments& args)
  {
      ...
      sess->buffer_data = Buffer::Data(buffer_obj);
      sess->buffer_length = Buffer::Length(buffer_obj);
#else
      Buffer *buffer_obj = ObjectWrap::Unwrap<Buffer>(args[0]->ToObject());
      sess->buffer_data = buffer_obj->data();
      sess->buffer_length = buffer_obj->length();
#endif
  ...
  static Handle<Value> Stats(const Arguments& args)
  {
      ...
      if (pcap_stats(sess->pcap_handle, &ps) == -1) {
...
Persistent<FunctionTemplate> PcapSession::s_ct;
